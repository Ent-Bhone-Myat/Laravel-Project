...
@section('breadc')
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
            ...
            <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Contact List</li>
        </ol>
        <h6 class="font-weight-bolder mb-0">Contact List - {{ $data->total() }}</h6>
    </nav>
@endsection
@section('content')
    ...
    @if (count($data) == 0)
        <h4 class="text-center">There is no <span class="text-info">Contact Data !</span></h4>
            ...
            <table class="table table-striped shadow p-3 mb-5 bg-white rounded text-center">
                ...
                <tbody>
                    @foreach ($data as $contact)
                        <tr>
                            <th scope="row" class="col-lg-1">{{ $contact->id }}</th>
                            <td class="col-lg-2">{{ $contact->name }}</td>
                            <td class="col-lg-2">{{ $contact->email }}</td>
                            <td class="col-lg-1">{{ $contact->created_at->format('j / F / Y') }}</td>
                            <td class="col-lg-1">
                                <a href="{{ route('contact.detail', $contact->id) }}">
                                    <button class="btn me-2" title="detail contact" style="background-color: blue">
