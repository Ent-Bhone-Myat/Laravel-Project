...
@section('breadc')
    ...
    <nav aria-label="breadcrumb">
        ...
        <h6 class="font-weight-bolder mb-0">User List - {{ $data->total() }}</h6>
    </nav>
@endsection
@section('content')
    ...
    @if (count($data) == 0)
        <h4 class="text-center">There is no <span class="text-info">User Data !</span></h4>
            ...
            <table class="table table-striped shadow p-3 mb-5 bg-white rounded text-center">
                ...
                <tbody>
                    @foreach ($data as $user)
                        <tr>
                            <th scope="row" class="col-lg-1">{{ $user->id }}</th>
                            <td class="col-lg-2">{{ $user->name }}</td>
                            <td class="col-lg-2">{{ $user->email }}</td>
                            <td class="col-lg-1">{{ $user->created_at->format('j / F / Y') }}</td>
                            <td class="col-lg-1">
                                <a href="{{ route('user.detail', $user->id) }}">
                                    <button class="btn me-2" title="detail user account" style="background-color: blue">
