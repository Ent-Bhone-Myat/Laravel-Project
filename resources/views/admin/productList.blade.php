...
@section('breadc')
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
            ...
            <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Product List</li>
        </ol>
        <h6 class="font-weight-bolder mb-0">Product List - {{ $data->total() }}</h6>
    </nav>
@endsection
@section('content')
    ...
    @if (count($data) == 0)
        <h4 class="text-center">There is no <span class="text-info">Product Data !</span></h4>
            ...
            <table class="table table-striped shadow p-3 mb-5 bg-white rounded text-center">
                ...
                <tbody>
                    @foreach ($data as $product)
                        <tr>
                            <th scope="row" class="col-lg-1">{{ $product->id }}</th>
                            <td class="col-lg-2">{{ $product->name }}</td>
                            <td class="col-lg-2">{{ $product->category_name }}</td>
                            <td class="col-lg-1">{{ $product->created_at->format('j / F / Y') }}</td>
                            <td class="col-lg-2">
                                <a href="{{ route('product.detail', $product->id) }}">
                                    <button class="btn me-2" title="detail product" style="background-color: blue">
